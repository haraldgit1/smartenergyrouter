...
# VergleichsBase-Line
from model_baseline import load_baseline_model, call_baseline
...
def get_forecast(
    ...
    backend: str = Query(BACKEND_NAME, description="tirex_v1 oder baseline_v1"),
):
    if MODEL is None:
    ...
    try:
        # Original ohne Vergleich BaseLine: preds_norm = call_tirex(MODEL, X, horizon_steps=horizon_steps)

        if backend == "baseline_v1":
            baseline = load_baseline_model()
            preds_norm = call_baseline(baseline, X, horizon_steps=horizon_steps)
            backend_used = "baseline_v1"
        else:
            preds_norm = call_tirex(MODEL, X, horizon_steps=horizon_steps)
            backend_used = "tirex_v1"
